  ...
  if (reviewArea) {
    reviewArea.innerHTML = builtQuiz.map((q, i) => {
      ...
      const explHTML = q.explanation && q.explanation !== "No explanation provided."
        ? `<details class="group mt-3 border border-indigo-100 dark:border-slate-700 bg-indigo-50/50 dark:bg-slate-800/80 rounded-xl overflow-hidden transition-all duration-200">
             <summary class="flex items-center justify-between p-3 text-xs font-bold text-indigo-700 dark:text-indigo-300 cursor-pointer select-none hover:bg-indigo-100/50 dark:hover:bg-slate-700/50 transition">
               ...
               <svg class="w-3.5 h-3.5 text-indigo-400 transform group-open:rotate-180 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
             </summary>
             <div class="px-3.5 pb-3.5 pt-1.5 border-t border-indigo-100 dark:border-slate-700 text-xs sm:text-sm text-indigo-900 dark:text-indigo-200 leading-relaxed">
